...
## load cleaned data
data = pd.read_table(snakemake.input.cleaned_data, sep='\t', index_col=0)
...
df_raw = pd.read_csv(snakemake.input.raw_data, sep='\t', index_col=0)
...
fig.update_yaxes(title_text='Feature abundance', row=1, col=1)
fig.update_yaxes(title_text='Number of detected features', row=2, col=1)
